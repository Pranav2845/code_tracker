...
export async function fetchCode360ProfileTotalCount(username) {
  const { data } = await axios.get(`/api/user/code360/count/${encodeURIComponent(username)}`);
  return data.totalCount;
  }

export async function fetchCode360SolvedProblems(username) {
  const { data } = await axios.get(`/api/user/code360/problems/${encodeURIComponent(username)}`);
